...
namespace Sunset.NewCourse
{
    public partial class GraduationPlanEditor : UserControl, IGraduationPlanEditor
    {
        ...
        private const string SubjectKey = "SubjectOrdinal";
        private const string DomainKey = "DomainOrdinal";
        ...
        private void LoadSubjectAndDomain()
        {
            ConfigData cd = K12.Data.School.Configuration[ConfigName];
            if (cd.Contains(SubjectKey))
            {
                XmlElement element = cd.GetXml(SubjectKey, XmlHelper.LoadXml("<Subjects/>"));
            ...
            if (cd.Contains(DomainKey))
            {
                XmlElement element = cd.GetXml(DomainKey, XmlHelper.LoadXml("<Domains/>"));
