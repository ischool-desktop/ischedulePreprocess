...
{
    public partial class SHGraduationPlanEditor : UserControl,IGraduationPlanEditor
    {
            ...
            if (source != null)
            {
                foreach (XmlNode node in source.SelectNodes("Subject"))
                {
                    ...
                    else
                    {
                        ...
                        row.CreateCells(dataGridViewX1, "", null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null);
                        dataGridViewX1.Rows.Add(row);
                        row.Cells[_CategoryIndex].Value = element.GetAttribute("Category");
                        row.Cells[_DomainIndex].Value = element.GetAttribute("Domain");
                        row.Cells[_RequiredByIndex].Value = element.GetAttribute("RequiredBy");
                        row.Cells[_RequiredIndex].Value = element.GetAttribute("Required");
                        row.Cells[_SubjectNameIndex].Value = element.GetAttribute("SubjectName");
                        ...
                        row.Cells[_StartLevelIndex].Value = element.GetAttribute("Level");
                        //填入自動開課資料(年級學期學分數)
                        int gradeyear = 0;
                        int semester = 0;
                        if (int.TryParse(element.GetAttribute("GradeYear"), out gradeyear) && int.TryParse(element.GetAttribute("Semester"), out semester))
                            row.Cells[(gradeyear - 1) * 2 + semester + _CreditStartIndex-1].Value = element.Attributes["Credit"].InnerText;
                        #endregion
                        //呼叫結束編輯處理函式
                        dataGridViewX1_CellEndEdit(this, new DataGridViewCellEventArgs(_SubjectNameIndex, row.Index));
