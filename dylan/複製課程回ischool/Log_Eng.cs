...
namespace Sunset.NewCourse
{
    ...
    {
        List<SimpleCourse> _NewK12Courses { get; set; }
        ...
        Dictionary<string, SimpleCourse> _K12CourseDic { get; set; }
        ...
        /// 設定新增課程的Log清單
        /// </summary>
        public void SetCourseLog(List<SimpleCourse> NewK12Courses)
        {
            _NewK12Courses = NewK12Courses;

            _K12CourseDic = new Dictionary<string, SimpleCourse>();
            foreach (SimpleCourse each in _NewK12Courses)
            {
        ...
        public string GetCourseLog()
        {
            ...
            foreach (SimpleCourse each in _NewK12Courses)
            {
                string school = each.SchoolYear;
                string semester = each.Semester;
                Log_Course.AppendLine(string.Format("學年度「{0}」學期「{1}」課程名稱「{2}」", school, semester, each.CourseName));
            }
            ...
            if (_TCInstructs != null)
            {
                foreach (TCInstructRecord each in _TCInstructs)
                {
                    if (_TeacherDic.ContainsKey(each.RefTeacherID) && _K12CourseDic.ContainsKey(each.RefCourseID))
                    {
                        ...
                        SimpleCourse cr = _K12CourseDic[each.RefCourseID];
                        string school = cr.SchoolYear;
                        string semester = cr.Semester;
                        string CourseName = string.Format("學年度:{0}　學期:{1}　課程名稱:{2}", school, semester, cr.CourseName);
        ...
        public void GetSCAttendLog()
        {
            if (_SCAttendDic != null)
            {
                ...
                foreach (string courseID in _SCAttendDic.Keys)
                {
                    ...
                    {
                        SimpleCourse cr = _K12CourseDic[courseID];
                        string school = cr.SchoolYear;
                        string semester = cr.Semester;
                        string CourseName = string.Format("學年度「{0}」學期「{1}」課程名稱「{2}」", school, semester, cr.CourseName);
