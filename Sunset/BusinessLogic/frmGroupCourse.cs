...
namespace Sunset
{
    ...
    /// 群組課程
    /// </summary>
    public partial class frmGroupCourse : FISCA.Presentation.Controls.BaseForm
    {
        ...
        public frmGroupCourse(List<string> CourseIDs)
        {
            InitializeComponent();
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            string strCondition = "$scheduler.course_section.ref_course_id in (" + string.Join(",", mCourseIDs.ToArray()) + ")";
            string strSQL = "select $scheduler.course_section.uid,$scheduler.course_section.ref_course_id,course_group from $scheduler.course_section inner join $scheduler.course_extension on $scheduler.course_section.ref_course_id=$scheduler.course_extension.ref_course_id where course_group='" + GroupName + "' or " + strCondition;
            ...
            {
                K12.Presentation.NLDPanels.Course.AddToTemp(NotAssignCourseIDs);
            ...
            List<CourseSection> Sections = mHelper.Select<CourseSection>("uid in ("+ string.Join(",",CourseSectionIDs.ToArray()) +")");

            Tuple<bool,string> Result = Sections.IsSameCourseGroup();
            ...
            //實際群組課程
            SunsetBL.GroupCourse(mCourseIDs, GroupName);
        }
        ...
        private void frmGroupCourse_Load(object sender, EventArgs e)
        {
            ...
            DataTable table = helper.Select("select distinct course_group from $scheduler.course_extension");
